...
const backgroundImages = [
    'url(../imge/tour.jpg)',
    'url(../imge/mountain.png)',
    'url(../imge/desert.png)',
    'url(../imge/bluesea.png)',
];
...
function changeBackground() {
    ...
    mainElement.style.backgroundImage = backgroundImages[currentIndex];
    mainElement.style.filter = 'blur(0.5px)'; // 배경 흐리게 설정
}

// 버튼 클릭 시 애니메이션 효과
function animateButton(button) {
    button.style.transform = 'scale(1.2)'; // 버튼 크기 증가
    setTimeout(() => {
        button.style.transform = 'scale(1)'; // 원래 크기로 돌아감
    }, 100); // 0.1초 후 원래 크기로 돌아감
}

// 이전 버튼 클릭 이벤트
document.querySelector('.prevButton').addEventListener('click', (event) => {
    animateButton(event.currentTarget); // 버튼 애니메이션
    currentIndex = (currentIndex - 1 + backgroundImages.length) % backgroundImages.length; // 인덱스 감소
    changeBackground();
});
...
document.querySelector('.nextButton').addEventListener('click', (event) => {
    animateButton(event.currentTarget); // 버튼 애니메이션
    currentIndex = (currentIndex + 1) % backgroundImages.length; // 인덱스 증가
    changeBackground();
});
